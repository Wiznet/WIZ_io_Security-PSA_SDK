...
static protocol_mode_t protocol_mode = 0U;
...
static void FW_UPDATE_PrintProtocol(void);
    ...
    if (COM_Receive(&key, 1U, RX_TIMEOUT) == HAL_OK) {
      switch (key) {
        case '1' :
          ...
          protocol_mode = uart_ymodem;
          exit = 1;
          break;
        case '2' :
          printf("  -- Protocol == TCP Server -- \r\n\n");
          protocol_mode = tcp_server;
          exit = 1;
          break;
        case '3' :
           printf("  -- Protocol == HTTP Client -- \r\n\n");
          protocol_mode = http_client;
          exit = 1;
          break;
        default:
...
/**
  * @brief  Display the FW_UPDATE protocol choices on HyperTerminal
  * @param  None.
  * @retval None.
  */
static void FW_UPDATE_PrintProtocol(void)
...
static HAL_StatusTypeDef FW_UPDATE_DownloadNewFirmware(SFU_FwImageFlashTypeDef *pFwImageDwlArea)
{
  ...
  else if (protocol_mode == tcp_server){
    uint8_t destip[4];
    uint16_t destport;
    
    while (exit == 0){
      switch(getSn_SR(DOWNLOAD_SOCKET)) {
        case SOCK_ESTABLISHED :
          //if(getSn_IR(DOWNLOAD_SOCKET) & Sn_IR_CON) {
          ctlsocket(DOWNLOAD_SOCKET, CS_GET_INTERRUPT, &inter);
          if(inter & Sn_IR_CON) {
            getsockopt(DOWNLOAD_SOCKET, SO_DESTIP, destip);
            getsockopt(DOWNLOAD_SOCKET, SO_DESTPORT, &destport);
            printf("%d:Connected - %d.%d.%d.%d : %d\r\n", DOWNLOAD_SOCKET, destip[0], destip[1], destip[2], destip[3], destport);
            setSn_IR(DOWNLOAD_SOCKET,Sn_IR_CON);
          }
          ret = Eth_Receive(&u_fw_size, pFwImageDwlArea->DownloadAddr);
          exit = 1;
          break;
        
        case SOCK_CLOSE_WAIT :
          eth_ret = disconnect(DOWNLOAD_SOCKET);
          printf("%d:Socket Closed ret = %d\r\n", eth_ret);
          break;
        
        case SOCK_INIT :
          eth_ret = listen(DOWNLOAD_SOCKET);
          printf("%d:Listen, TCP server, port [%d], ret = %d\r\n", DOWNLOAD_SOCKET, TCPS_PORT, eth_ret);
          printf("Waiting for connection from client\r\n");
          break;
        
        case SOCK_CLOSED:
          eth_ret = socket(DOWNLOAD_SOCKET, Sn_MR_TCP, TCPS_PORT, 0x00);
          printf("%d:Socket opened ret = %d\r\n", DOWNLOAD_SOCKET, eth_ret);
          break;
...
/**
  * @brief  Ethernet Data Packet Transfer completed callback.
  * @param  pData Pointer to the buffer.
  * @param  uSize Packet dimension (Bytes).
  * @retval None
  */
HAL_StatusTypeDef Eth_DataPktRxCpltCallback(uint8_t *pData, uint32_t uFlashDestination, uint32_t uSize)
{
  int32_t ret;
