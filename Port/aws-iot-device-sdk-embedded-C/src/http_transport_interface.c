...
/* SSL context */
tls_context_t *g_http_tls_context_ptr;
...
int32_t http_send_request(TransportInterface_t *pTransportInterface, uint8_t *buffer, char *pMethod, http_config_t *http_config)
{
    ...
    response.getTime = (HTTPClient_GetCurrentTimeFunc_t)HAL_GetTick;
...
#if (_WIZCHIP_ == W6100)
    ret = connect(g_http_network_context.socketDescriptor, http_config->http_ip, http_config->http_port, 4);
#else
...
int32_t http_get(uint8_t sock, uint8_t *buffer, char *http_url, tls_context_t *tls_context)
{
    int32_t ret = -1;
    ...
    if (!is_ipaddr(g_http_config.http_domain, g_http_config.http_ip)) // IP
    {
        uint8_t dns_buf[512];
#if (_WIZCHIP_ == 6100)
        ret = get_ipaddr_from_dns(g_http_config.http_domain, g_http_config.http_ip, dns_buf);
#else
...
int32_t http_post(uint8_t sock, uint8_t *buffer, char *http_url, tls_context_t *tls_context)
{
    int32_t ret = -1;
    uint8_t *pPath, *pDomain;
    uint32_t pPathLen, pDomainLen;
...
/* HTTPS */
int8_t https_connect(uint8_t sock, http_config_t *http_config, tls_context_t *tls_context)
{
    int8_t ret = -1;
