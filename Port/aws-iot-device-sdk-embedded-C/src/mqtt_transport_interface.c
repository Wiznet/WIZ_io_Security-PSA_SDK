...
/* SSL context pointer */
tls_context_t *g_mqtt_tls_context_ptr;
...
int8_t mqtt_transport_init(uint8_t cleanSession, uint8_t *ClientId, uint8_t *userName, uint8_t *password, uint32_t keepAlive)
{
    ...
    g_mqtt_config.mqtt_connect_info.pClientIdentifier = (const char *)ClientId;
    ...
    g_mqtt_config.mqtt_connect_info.pUserName = (const char *)userName;
    if (userName == NULL)
        g_mqtt_config.mqtt_connect_info.userNameLength = 0;
    else
        g_mqtt_config.mqtt_connect_info.userNameLength = strlen((const char *)userName);

    g_mqtt_config.mqtt_connect_info.pPassword = (const char *)password;
    if (password == NULL)
        g_mqtt_config.mqtt_connect_info.passwordLength = 0;
    else
        g_mqtt_config.mqtt_connect_info.passwordLength = strlen((const char *)password);
...
int8_t mqtt_transport_connect(uint8_t sock, uint8_t ssl_flag, uint8_t *recv_buf, uint32_t recv_buf_len, uint8_t *domain, uint32_t port, tls_context_t *tls_context)
{
    ...
    if (!is_ipaddr(domain, g_mqtt_config.mqtt_ip)) // IP
    {
        uint8_t dns_buf[512];
#if (_WIZCHIP_ == W6100)
        ret = get_ipaddr_from_dns(domain, g_mqtt_config.mqtt_ip, dns_buf);
#else
...
#if (_WIZCHIP_ == W6100)
        ret = connect(sock, g_mqtt_config.mqtt_ip, port, 4);
#else
    ...
    else
    {
        ...
        ret = ssl_transport_init(tls_context, (int *)(intptr_t)sock, (const char *)domain);
        if (ret != 0)
        {
            mqtt_transport_close(sock, &g_mqtt_config);
        ...
        ret = ssl_socket_connect_timeout(tls_context, (char *)g_mqtt_config.mqtt_ip, port, 0, MQTT_TIMEOUT);
...
int mqtt_transport_publish(uint8_t *pub_topic, uint8_t *pub_data, uint32_t pub_data_len, uint8_t qos)
{
    ...
    g_mqtt_config.mqtt_publish_info.pTopicName = (const char *)pub_topic;
    g_mqtt_config.mqtt_publish_info.topicNameLength = strlen((const char *)pub_topic);
